...
class DemoRunner:
    ...
    def _create_ric(self):
        ...
        for ric_impl in self._cfg.programs.get_ric_programs():
            if ric_impl.ric_implementation is not None and ric_impl.ric_implementation == self._cfg.get_used_ric().implementation:
        ...
        if not ric_added:
            logging.warning(f"No RIC for implementation {self._cfg.get_used_ric().implementation.value} found! No RIC will be started.")
    ...
    def _create_core(self):
        ...
        for core_impl in self._cfg.programs.get_core_programs():
            if core_impl.core_implementation is not None and core_impl.core_implementation == self._cfg.get_used_core().implementation:
        ...
        if not core_added:
            logging.warning(
                f"No Core for implementation {self._cfg.get_used_core().implementation.value} found! No Core will be started.")
    ...
    def _create_gnb(self):
        ...
        for gnb_impl in self._cfg.programs.get_gnb_programs():
            if gnb_impl.gnb_implementation is not None and gnb_impl.gnb_implementation == self._cfg.get_used_gnb().implementation:
        ...
        if not gnb_added:
            logging.warning(
                f"No gNB for implementation {self._cfg.get_used_gnb().implementation.value} found! No gNB will be started.")
