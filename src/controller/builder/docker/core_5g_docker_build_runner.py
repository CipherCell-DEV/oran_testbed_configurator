...
class Core5GDockerBuildRunner(DockerBuilderBase):
    ...
    def build(self, log_buffer: Optional[LogQueue] = None) -> bool:
        ...
        if self.setup_cfg.get_used_core().implementation == CoreImplementation.OPEN5GS_SRS:
            result = self.docker_compose_build_helper('5gc', ["docker", "compose", "build", '5gc'], log_buffer)
            os.chdir(curr_dir)
            return result
        if self.setup_cfg.get_used_core().implementation == CoreImplementation.OPEN5GS:
