...
class NearRTRICPatcher(SinglePatcherBase):
    ...
    def patch(self):
        ...
        if os.path.exists(self._patch_file_path):
            if self._setup_cfg.get_used_ric().implementation == RICImplementation.ORAN_SC_RIC or \
                    self._setup_cfg.get_used_ric().implementation == RICImplementation.FLEX_RIC:
                if self._setup_cfg.get_used_ric().build_type == BuildType.DOCKER:
    ...
    def patch_config_file(self):
        if self._setup_cfg.get_used_ric().implementation == RICImplementation.FLEX_RIC:
            ...
            config = {'e2term': {
                "ip": getattr(self._setup_cfg.get_used_ric().ip_config, 'e2term_ip')
            }}
    ...
    def patch_docker_compose(self) -> Optional[dict]:
        ...
        if self._setup_cfg.get_used_ric().implementation == RICImplementation.ORAN_SC_RIC:
            ...
            config = {
                ...
                'ric': {
                    "subnet": self._setup_cfg.get_used_ric().ip_config.subnet
                }
            }

            for service, (env_var, ip_attr) in ORAN_SC_RIC_SERVICE_IP_MAP.items():
                ip_value = getattr(self._setup_cfg.get_used_ric().ip_config, ip_attr)
    ...
    def copy_config_files(self):
        if self._setup_cfg.get_used_ric().implementation == RICImplementation.ORAN_SC_RIC:
            ...
            logging.error(f"{str(self._setup_cfg.get_used_ric().implementation.value)} is not implemented yet.")
            exit(1)
